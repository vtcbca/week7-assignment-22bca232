...
with open('C:\\22bca232\\python\\product_sell.csv','w',newline='')as f:
    obj1=csv.writer(f)
    col=['Prod_No', 'Prod_Name',' Jan','Feb','Mar','Apr','May','Jun']
    obj1.writerow(col)
    
with open('C:\\22bca232\\python\\prodcut_sell.csv','a',newline='')as cf:
    obj1=csv.writer(cf)
    list=[]
    for i in range(6):
...
# 2. Create dataframe
df=pd.read_csv('C:\\22bca232\\python\\product_sell.csv')
df
...
#12.Print only those product id , product name where january sell is > 5000 and  and February sell is >8000:
